...
3. Check you're on the correct branch from PRD `branchName`. If not, check it out or create from main. **Branch names must use `feat/` prefix** (e.g. `feat/stylesheet-blast-radius`). Never use `ralph/` for git branches — see `.cursor/rules/ralph-branch-naming.mdc`.
