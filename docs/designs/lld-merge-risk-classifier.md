...
- **Dogfood.** This repo's **`.merge-risk.yml`** enables **`file_patterns`**, **`author_seniority`**, **`diff_size`**, **`branch_age`**, and **`service_criticality`** (with root **`services`**) so merge-risk scoring on our own pull requests exercises path rules (notably the committed GitHub Action bundle under `extensions/github-action/dist/` and files under `schema/`), login-tier rules (including common automation accounts), head-commit age from adapter-hydrated timestamps, and service-level globs.
