...
│   │   └── cobertura.ts
│   ├── mutators/
│   │   ├── builtins.ts            # Brindle: registers shipped mutators (LLD: registry.ts + examples)
│   │   ├── mutatorPrimitives.ts   # Brindle: shipped (shared conditional multiplier factory)
│   │   ├── juniorAuthor.ts        # Brindle: shipped (`junior_author`)
│   │   ├── juniorAuthor.types.ts
│   │   ├── criticalService.ts     # Brindle: shipped (`critical_service`)
│   │   └── criticalService.types.ts
│   ├── rules/
│   │   ├── declarativeRule.ts     # Brindle: shipped (labels_any MVP; ADR 0001)
│   │   └── declarativeRule.types.ts
│   ├── plugins/
│   │   ├── loadTrustedPlugins.ts       # Brindle: shipped (labels_any MVP; ADR 0001)
│   │   ├── trustedPluginPaths.ts
│   │   ├── trustedPlugins.types.ts
│   │   └── trustedPluginDocument.types.ts
│   ├── config.ts
│   └── report.ts                  # builds platform-neutral RiskReport
│   ├── serviceCatalog/
│   │   └── globMatchForServices.ts   # Brindle: shipped (shared `services` glob matching)
├── adapters/
...
- **Coverage artifacts.** **`core/coverage/adapter.ts`** + **`adapter.types.ts`** centralize parsing UTF-8 artifact text into **`CoverageReport`** (**Istanbul-only** MVP via **`istanbul.ts`**; lcov/Cobertura deferred). The GitHub adapter calls the adapter when hydrating **`PRContext.coverage`** from the PR head ref (ADR 0005).
- **Mutators.** **`core/mutators/builtins.ts`** registers **`junior_author`** and **`critical_service`** (with **`mutatorPrimitives.ts`**). JSON Schema validates **`mutators.junior_author.options`** and **`mutators.critical_service.options`** when those keys are present (`logins` / **`multiplier`** for the former; **`service_ids`** / **`multiplier`** for the latter; multipliers strictly &gt; 1).
...
Identical to v3 and platform-neutral already. Read from the base branch by the adapter. The full schema with criteria, mutators, services, declarative rules, trusted plugins, and the auto-merge block is unchanged at the v3 level of ambition. **In Brindle today:** `schema/merge-risk-config.schema.json` validates a **subset** of that surface (thresholds, criteria, mutators, auto-merge); when **`criteria.file_patterns`** is present, its **`options`** are validated (`patterns` with `glob` / `score`, optional `aggregation: max`). When **`criteria.author_seniority`** is present, its **`options`** are validated (`rules` with `login` / `score`, optional `default_score` and `aggregation: max`). When **`criteria.service_criticality`** is present, its **`options`** are validated (`scores`, optional `aggregation: max`, optional `default_score`). When **`criteria.branch_age`** is present, its **`options`** are validated (`max_age_hours_for_cap` positive when set). When **`mutators.junior_author`** is present, its **`options`** are validated (`logins` non-empty strings, **`multiplier`** strictly greater than 1). When **`mutators.critical_service`** is present, its **`options`** are validated (**`service_ids`** non-empty strings, **`multiplier`** strictly greater than 1). Top-level **`services`** entries are validated (`globs` arrays) when present. When **`declarative_rules`** is present, each rule entry is validated (`weight`, optional `enabled`, optional **`options`** with **`labels_any`** string array and **`score`** 0–100). When **`trusted_plugins`** is present, **`directory`** (non-empty string) and **`paths`** (array of non-empty strings) are validated; the GitHub Action rejects any resolved path not strictly under **`directory`**. Repository-relative `services` globs and file patterns mean the same thing on every platform.
