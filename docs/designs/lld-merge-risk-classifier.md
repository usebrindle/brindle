...
│   │   └── criticalService.ts     # not yet in Brindle
│   ├── rules/
...
- **Mutators.** **`core/mutators/builtins.ts`** registers **`junior_author`** (**`juniorAuthor.ts`** + **`juniorAuthor.types.ts`**) and shared **`mutatorPrimitives.ts`**. JSON Schema validates **`mutators.junior_author.options`** when that key is present (`logins`, **`multiplier`** strictly &gt; 1). **`criticalService`** / **`critical_service`** is not present yet.
